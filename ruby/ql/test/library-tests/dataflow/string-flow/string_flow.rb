...
def m_dump
    a = source "a"
    b = sink a.dump # $ hasTaintFlow=a
    sink b.undump # $ hasTaintFlow=a
end

def m_each_line
    a = source "a"
    b = a.each_line { |line| sink line } # $ hasValueFlow=a
    sink b # $ hasValueFlow=a
    c = a.each_line
    sink c.to_a[0] # $ hasValueFlow=a
end

def m_lines
    a = source "a"
    b = a.lines { |line| sink line } # $ hasValueFlow=a
    sink b # $ hasValueFlow=a
    c = a.lines
    sink c[0] # $ hasValueFlow=a
end
