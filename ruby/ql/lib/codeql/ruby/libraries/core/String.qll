...
module String {
  ...
  private class EachLineSummary extends SummarizedCallable {
    ...
    override predicate propagatesFlowExt(string input, string output, boolean preservesValue) {
      // Technically `each_line` strips newline characters and so doesn't preserve
      // value, but it's near enough to probably be OK.
  ...
  private class ScanSummary extends SummarizedCallable {
    ...
    override predicate propagatesFlowExt(string input, string output, boolean preservesValue) {
      ...
      (
        ...
        preservesValue = true
        or
        output = "Parameter[?] of BlockArgument" and preservesValue = false
      )
    }
  }
